# Security Policy

## Supported versions

Only the latest released version receives security fixes.

| Version | Supported |
|---|---|
| 0.2.x   | Yes |
| < 0.2   | No  |

## Reporting a vulnerability

Report privately — never in a public issue, discussion, or pull request:

- Email: **xineraman8@gmail.com**

Include what you found, the affected version, and reproduction steps if possible. Expect an initial response within 5 business days.

A fix ships before any public disclosure. The changelog entry (and a GitHub security advisory, where applicable) for the fixed version describes the issue once the patched release is available.
